...
def example_6 (time_stamp_value):
    ...
    time1_tm = time_subroutines.tm (tm_year = 1972 - 1900, tm_mon = 1 - 1,
                                    tm_mday = 1, tm_hour = 0, tm_min = 0,
                                    tm_sec = 0, tm_isdst = 0, tm_gmtoff = 0,
                                    tm_zone = None)
    ...
    time_subroutines.time_UTC_add_seconds (time2_tm,
                                           time_stamp_value - 63072010,
                                           time_subroutines.int_min)

    # Convert to local time.
    time3_tm = time_subroutines.tm()
    time_subroutines.time_current_tm(time2_tm)
    pprint.pprint(time.gmtime())
    print(str(time2_tm))
    time_subroutines.time_UTC_to_local (time2_tm, time3_tm,
                                        time_subroutines.int_min)
    print(str(time3_tm))
    # Convert the POSIX tm structure to the Python struct_time structure.
    # The following conversion is based on comparing the struct_time
    # structure returned by time.localtime with
    time4 = time.struct_time((time3_tm.tm_year + 1900, time3_tm.tm_mon + 1,
    ...
    print ("PTP " + str(time_stamp_value) + " displays as " + buffer1 + ".")
    return
